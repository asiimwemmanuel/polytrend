...
def assert_processed_shape(result, expected_n: int):
    ...
    assert len(data_points) == expected_n, f"expected {expected_n} points, got {len(data_points)}"
...
class TestCSVInput:
    ...
    def test_three_column_csv_preserves_errors(self):
        ...
        assert all(err == 1.0 for err in errors), "uniform error of 1.0 should be read from CSV"
