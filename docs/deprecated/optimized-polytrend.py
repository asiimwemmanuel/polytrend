...
class PolyTrend:
  ...
  def polyfind(
    self, known_data: List[Tuple[float, float]], degrees: List[int]
    ...
    return lambda x_vals: np.array(
      reg.predict(poly_features.transform(np.array(x_vals).reshape(-1, 1)))
    )
  ...
  def graph(
    ...
    if extrap_data:
      ...
      x_func = np.linspace(
        min(np.concatenate((x_known, x_extrap))),
        max(np.concatenate((x_known, x_extrap))),
        100,
      )
      y_func = func(x_func)
